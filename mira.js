...
function getPlaybook() {
  return loadJSON(PLAYBOOK_FILE, {
    focus_this_period: '(尚未優化。預設：台南手工職人精品服務、不叫賣、雙主力組合推薦、送禮場景引導。)',
...
// ── 內容類型定義 ──
const TYPES = {
  greeting: { label: '門市話術庫', ask: '產出分情境的門市店員話術：①迎賓開場 ②商品介紹(胖卡龍/費南雪雙主力) ③推薦組合 ④破解疑慮(嫌貴/送禮不知怎麼選/比較) ⑤臨門收單。每段給「情境→話術→為什麼」。' },
  upsell: { label: '加購腳本', ask: '產出門市加購腳本：列出 5-6 個加購時機(例:客人買 6 入時、挑禮盒時、提到送禮時)，每個給觸發訊號 + 自然不壓迫的加購話術 + 預期效果。涵蓋雙享禮盒、禮盒升級、季節限定搭配、企業/婚禮大單引導。' },
  training: { label: '新人教育訓練', ask: '產出新進店員上手教材：①產品知識(口味/保存期限/過敏原) ②品牌故事(台南手工職人精品、台南起家、雙主力) ③門市 SOP 重點 ④常見客訴與處理。寫成可直接給新人讀的教材。' },
...
2. 話術要短、口語、有溫度，符合台南手工職人精品語氣。
3. 緊扣雙主力(胖卡龍+費南雪)與送禮場景(婚禮喜餅/企業/犒賞自己)。
