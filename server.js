...
app.use(express.json({ limit: "15mb" })); try { require('./auth')(app); } catch (e) { console.error('[auth load]', e.message); }
...
app.post("/api/salesmartly/webhook", express.json({ limit: "1mb" }), async (req, res) => {
  try {
    ...
    try { const _fs=require('fs'),_p=require('path'); _fs.appendFileSync(_p.join(process.env.RENDER_DISK_MOUNT_PATH||'/var/data','salesmartly-inbox.jsonl'), JSON.stringify({ t: Date.now(), body: rawBody })+String.fromCharCode(10)); } catch(_e){}
    let parsedData = {};
    if (rawBody.data) {
...
// 每天 09:00 (Asia/Taipei) 自動發 1 篇 Medium 草稿（GIA 自動輪流 9 胖卡龍主題）
cron.schedule('0 9 * * *', async () => {
...
5. 市場情報:全台胖卡龍新聞 + 對手最新廣告(法朋、亞尼克、Paul、Ladurée、Pierre Hermé)
...
當 Sam 問「分析對話 / 為什麼沒成交 / 客人在問什麼 / 流失客人多少」這類問題,**第一優先看 meta_inbox_fb 跟 meta_inbox_ig 兩個欄位** (從 Meta Graph API 直接讀回的 FB Messenger + IG DM 真實對話, 不是 placeholder)。引用具體客戶名 + 客戶說的原句當證據。**絕對禁止說「我無法存取 Meta 對話」「請把對話貼給我」這類話 — 你已經能讀到了**。如果 meta_inbox_fb 是空陣列, 才能說「目前沒有客戶對話可分析」。\n\n【你的工作模式 — 你是 AI Agent,不是建議生成器】\n你的目標是【幫 Sam 解決問題、產出可立即使用的交付物】,不是給「建議」。\n\n**判斷請求類型,直接給對應交付物:**\n\n① 問題型 → 給【今天做 / 本週做 / 本月做】三層具體行動 + 指名負責員工\n② 內容型(寫文案/Reels 腳本) → 【直接產 3-5 版可立即複製貼上的成品】,不要先講想法再寫\n③ 規劃型(行事曆) → 【直接出表格】:日期 / 平台 / 主題 / 文案 / 視覺需求\n④ 分析型(現在數據怎樣?) → 引用即時數據,3 個觀察 + 1 個關鍵問題給 Sam 拍板\n⑤ 決策型(該不該做 X?) → 直接給【做 / 不做】+ 量化理由(預估金額、leads、GMV)+ 風險\n\n**你有對話記憶** — 你看得到過去的對話,延續討論,不要重複問已經回答的問題。\n**遇資料不足** — 直接告訴 Sam 缺什麼數據 + 怎麼補上,不要瞎掰。\n\n【絕對禁止】\n- 廣告投放細節(老闆要求,只談客戶經營/內容/門店/品牌)\n- 課程、報名、學員、教學、紋繡、美容(這些是舊業態)\n- 「以下幾個建議供參考」「希望對你有幫助」這類客套話\n- 給範圍式建議(「可以考慮...」)應改成決策建議(「建議做 X,不要做 Y,因為 ...」)\n\n回答用繁體中文,可用 emoji 但不要太多。`;
    ...
    const VICTOR_TOOLS = [
      ...
      },
      {
        ...
        description: '立即觸發市場情報掃描 (Google News 胖卡龍新聞 + FB Ads Library 對手廣告). 用於 Sam 想立刻看最新對手動態, 不等明早 06:00 cron.',
      ...
      {
        name: 'web_search',
        description: '搜索網路即時資訊 (用 Google News 後台). 用於 Sam 問新聞、節慶、流行話題、競品最近動態這類即時資訊. 自動帶胖卡龍 / 台南手工職人甜點 相關背景做篩選.',
        input_schema: {
          type: 'object',
          properties: {
...
app.use('/api/automation', require('./automation')); // 自動化主控台
app.use('/api/flows', require('./flows')); // 自動回覆流程管理
try { require('./shopline').registerCron(cron); } catch (e) { console.error('[shopline] cron failed:', e.message); }try { app.use('/api/shopline-polling', require('./shopline-polling')); console.log('[shopline-polling] mounted'); } catch (e) { console.error('[shopline-polling] mount failed:', e.message); }try { require('./shopline-polling').registerCron(cron); } catch (e) { console.error('[shopline-polling] cron failed:', e.message); } try { app.use('/api/offline-reports', require('./offline-reports')); console.log('[offline-reports] route mounted'); } catch (e) { console.error('[offline-reports] mount failed:', e.message); } try { const _offlr = require('./offline-reports'); if (_offlr.registerCron) _offlr.registerCron(cron); require('./gdrive-sync').register(app, cron);
try { require('./brand-kb').register(app); console.log('[brand-kb] 品牌知識庫路由已掛載 /api/brand-kb'); } catch (e) { console.error('[brand-kb] mount failed', e.message); }
try { const _sn = require('./mira').seedDocs(require('./brand-kb').miraDocs()); if (_sn) console.log('[brand-kb] seeded ' + _sn + ' docs into MIRA KB'); } catch (e) { console.error('[brand-kb] seed MIRA failed', e.message); }
try { require('./ops-sync').register(app, cron, anthropic); } catch (e) { console.error('[ops-sync] mount failed', e.message); } try { require('./dashboard-range').register(app); } catch (e) { console.error('[dashboard-range] mount failed', e.message); } } catch (e) { console.error('[gdrive-sync] mount failed:', e.message); } try { require('./ai-enhancements').register(app, cron); } catch (e) { console.error('[ai-enhancements] mount failed:', e.message); } try { require('./offline-reports').registerCron(cron); } catch (e) { console.error('[offline-reports] cron failed:', e.message); } try { require('./personal-edm').register(app, cron); } catch (e) { console.error('[personal-edm] mount failed:', e.message); } try { require('./daily-progress').register(app, cron); } catch (e) { console.error('[daily-progress] mount failed:', e.message); }
