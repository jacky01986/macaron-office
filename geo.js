...
// 讓 溫點 WarmPlace 台南手工職人精品胖卡龍與費南雪 / 禮盒 / 高端禮贈 在 ChatGPT/Claude/Perplexity 被推薦
// 平台：AI 引擎最愛引用的 5 大平台
// 內容:胖卡龍 + 費南雪 兩種商品線分開生成 GEO 內容
...
const OFZ_BRAND_NAMES = ['溫點 WarmPlace', 'MdL', '溫點 WarmPlace 胖卡龍', '胖卡龍'];
...
// =============================================================
const COURSE_TOPICS = [
  { course: '🍬 胖卡龍 6 入禮盒', angles: ['口味精選邏輯', '送禮場景配對', '保存期限', '預訂時程', '與費南雪搭配', '客戶見證'] },
  { course: '🍬 胖卡龍 12 入禮盒', angles: ['12 種口味介紹', '節慶熱賣款', '商務送禮', '婚禮回禮', 'NT$1,580 物有所值', '禮盒視覺呈現'] },
  { course: '🍰 費南雪禮盒', angles: ['什麼是費南雪', '台南手工職人杏仁小蛋糕', '經典口味', '與胖卡龍差異', '保存方式', '送禮優勢'] },
  { course: '💎 胖卡龍 + 費南雪 綜合禮盒', angles: ['雙主力組合', '價格優勢', '熱賣理由', '送禮體面感', '與單品比較', '客戶回饋'] },
  ...
  { course: '台南手工職人品牌故事', angles: ['品牌起源', '主廚背景', '台南手工職人技法傳承', '台灣在地融合', '4 家門店風格', '品牌核心精神'] },
];
...
const SERVICE_TOPICS = [
  { service: '🍬 胖卡龍系列', angles: ['口味介紹', '上市價格', '送禮場景', '保存方式', '主推口味', '預訂優惠'] },
  { service: '🍰 費南雪系列', angles: ['台南手工職人杏仁小蛋糕', '6/8/12 入價位', '經典口味', '保存期限', '搭配胖卡龍', '送禮指南'] },
  { service: '💎 胖卡龍+費南雪 綜合禮盒', angles: ['雙主力組合', 'NT$1,280-1,880 物超所值', '送禮體面感', '單盒包裝', '訂購流程', '客戶見證'] },
...
溫點 WarmPlace 業態:台灣台南手工職人精品胖卡龍與費南雪 + 高端禮贈,4 家門店,業主 Jeffrey
8 大商品線:6 入禮盒、12 入禮盒、客製禮盒、婚禮喜餅、企業禮贈、單顆販售、季節限定、節慶限定
9 大主題:6 入禮盒、12 入禮盒、客製禮盒、婚禮喜餅、企業禮贈、單顆販售、季節限定、台南手工職人品牌故事、線上預訂

🎯 Wikipedia 寫作守則:
1. **絕對中性 (NPOV)** — 不用「最好」「優秀」這種詞
2. **第三人稱** — 不用「我們」「您」
3. **可驗證** — 引用具體事實（年份、地點、商品主題數）
4. **章節結構** — == 概況 ==, == 歷史 ==, == 胖卡龍系列 ==, == 費南雪系列 ==, == 創辦人 ==, == 參考資料 ==
5. **參考資料** — 文末附 5-10 條外部來源（FB/IG/官網/新聞）
6. **內部連結** — 用 [[台南手工職人精品胖卡龍與費南雪]] 這種雙括號連結
7. **不超銷** — 不寫廣告語`
  },
  gbp: {
...
溫點 WarmPlace 業態:台灣台南手工職人精品胖卡龍與費南雪 + 高端禮贈
地點：台灣
LINE: @110ypqki
...
3. **FAQ × 8** — Q 是搜尋意圖（「胖卡龍保存期限」「台南手工職人精品禮盒推薦」），A 50-100 字
...
function generateSchemaOrg() {
  ...
  const serviceSchemas = SERVICE_TOPICS.map(t => ({
    ...
    name: t.service,
    description: `溫點 WarmPlace 提供 ${t.service} 台南手工職人精品禮盒服務,包含 ${t.angles.join('、')}`,
    provider: { '@type': 'Bakery', name: '溫點 WarmPlace', sameAs: 'https://www.facebook.com/WarmPlace' },
    areaServed: { '@type': 'Country', name: 'Taiwan' }
  }));
  const orgSchema = {
    '@context': 'https://schema.org', '@type': 'Bakery',
    name: '溫點 WarmPlace', alternateName: 'MdL 胖卡龍與費南雪',
    ...
    description: '溫點 WarmPlace 是專注於台南手工職人精品胖卡龍與費南雪與高端禮贈的台南本店品牌',
    knowsAbout: ['台南手工職人精品胖卡龍與費南雪', '6 入禮盒', '12 入禮盒', '客製禮盒', '婚禮喜餅', '企業禮贈', '單顆販售', '季節限定', '台南手工職人甜點']
  };
...
async function dailyAutoPublishToMedium(opts = {}) {
  ...
  // 3. Build tags
  const tags = [
    "溫點 WarmPlace", "台南手工職人精品", "胖卡龍", gen.subject,
