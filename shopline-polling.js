...
router.get('/probe-paths', async (req, res) => {  if (!OPEN_API_TOKEN) return res.json({ ok: false, error: 'no token' });  const paths = (req.query.paths || '').split(',').filter(Boolean);  if (!paths.length) return res.json({ ok: false, error: 'pass ?paths=p1,p2,p3 (without leading /v1)' });  const out = [];  for (const raw of paths) {    const p = raw.startsWith('/') ? raw : '/v1/' + raw;    try {      const url = OPEN_BASE + p + (p.includes('?') ? '&' : '?') + 'merchant_id=' + MERCHANT_ID;      const r = await fetch(url, { headers: { 'Authorization': 'Bearer ' + OPEN_API_TOKEN, 'Accept': 'application/json' } });      const text = await r.text();      out.push({ path: p, status: r.status, isJson: text.trim().startsWith('{') || text.trim().startsWith('['), sample: text.slice(0, 120) });    } catch (e) { out.push({ path: p, error: e.message.slice(0, 60) }); }  }  res.json({ ok: true, results: out });});// Friendly wrappers for discovered Shopline endpointsfunction _wrapGet(p){return async(req,res)=>{if(!OPEN_API_TOKEN)return res.json({ok:false,error:'no token'});try{const url=OPEN_BASE+'/v1/'+p+'?merchant_id='+MERCHANT_ID;const r=await fetch(url,{headers:{'Authorization':'Bearer '+OPEN_API_TOKEN,'Accept':'application/json'}});res.json(await r.json());}catch(e){res.status(500).json({ok:false,error:e.message})}};}['categories','promotions','staffs','warehouses','customer_groups','payments','webhooks'].forEach(p=>router.get('/'+p.replace(/_/g,'-'),_wrapGet(p)));// Test register webhook (GET for easy testing)router.get('/test-register-webhook', async (req, res) => {  if (!OPEN_API_TOKEN) return res.json({ok:false,error:'no token'});  const topic = req.query.topic || 'orders/create';  const callback_url = req.query.callback_url || 'https://macaron-office.onrender.com/api/shopline/webhook';  try {    const url = OPEN_BASE + '/v1/webhooks?merchant_id=' + MERCHANT_ID;    const r = await fetch(url, { method: 'POST', headers: { 'Authorization': 'Bearer ' + OPEN_API_TOKEN, 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify({ webhook: { topic, callback_url, format: 'json' } }) });    const text = await r.text();    let data; try { data = JSON.parse(text); } catch { data = { raw: text.slice(0, 500) }; }    res.json({ ok: r.ok, status: r.status, topic, callback_url, data });  } catch (e) { res.status(500).json({ ok: false, error: e.message }); }});router.get('/token-info', async (req, res) => {  if (!OPEN_API_TOKEN) return res.json({ ok: false, error: 'no token set' });  try {    const r = await fetch(OPEN_BASE + '/v1/token/info', { headers: { 'Authorization': 'Bearer ' + OPEN_API_TOKEN, 'Accept': 'application/json' } });    const text = await r.text();    let data; try { data = JSON.parse(text); } catch { data = { raw: text.slice(0, 500) }; }    res.json({ ok: r.ok, status: r.status, data });  } catch (e) { res.status(500).json({ ok: false, error: e.message }); }});function registerCron(cron) {
