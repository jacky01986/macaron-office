// offline-reports.js v5 — 線下報告中心 (檔案上傳 + Claude 萃取 + 月目標達成率 + AI 分析 + 從原始檔重新月結算)
...
router.post('/upload-file', upload.single('file'), async (req, res) => {
  try {
    ...
    } else { return res.status(400).json({ ok: false, error: '不支援的檔案類型: ' + (ext || mime) }); }
...
router.post('/submit', (req, res) => {
  try {
    ...
    const record = {
      ...
      revenue: Number(b.revenue) || 0, orders: Number(b.orders) || 0,
...
router.post('/targets', (req, res) => {
  try {
    const { branch, month, target } = req.body || {};
    if (!branch || !month) return res.status(400).json({ ok: false, error: 'branch + month required' });
    if (!/^\d{4}-\d{2}$/.test(month)) return res.status(400).json({ ok: false, error: 'month must be YYYY-MM' });
    const updated = setTarget(branch, month, target);
...
router.get('/branch-analysis/:branch', async (req, res) => {
  try {
    ...
    const ctx = '門市:' + wantedBranch + '\n累計營收:NT$' + totalRev.toLocaleString() + ' / 訂單' + totalOrd + ' / ' + all.length + ' 份報告\n\n月份明細(含目標達成率):\n' + monthList.map(m => '【' + m.month + '】營收 NT$' + m.revenue.toLocaleString() + (m.target > 0 ? ' / 目標 NT$' + m.target.toLocaleString() + ' = 達成 ' + m.achievement_pct + '%' : ' / 無目標') + ' / 訂單 ' + m.orders + ' / ' + m.count + ' 份報告 / 問題 ' + m.problems.length + ' 件').join('\n') + '\n\n所有問題點:\n' + allProblems.map(p => '• ' + p).join('\n') + '\n\n所有檢討觀察:\n' + allReviews.map(r => '• ' + r).join('\n') + '\n\n現有行動項目:\n' + allActions.map(a => '• ' + a).join('\n');
    ...
    const respText = (result.content || []).map(c => c.text || '').join('');
    let analysis = {};
    try { const m = respText.match(/\{[\s\S]*\}/); if (m) analysis = JSON.parse(m[0]); }
    catch (e) { analysis = { _raw: respText.slice(0, 500), _err: e.message }; }
...
// NEW: source aggregate - re-read raw files to extract daily revenue, aggregate by month
const SOURCE_AGGREGATE_SYS = "你是溫點 WarmPlace 烘焙坊的會計總監。從原始報告(尤其是 Excel)逐日萃取營收紀錄,再按月份結算。只回傳純 JSON,不要 markdown 標記。欄位:{\"months\":[{\"month\":\"YYYY-MM\",\"revenue\":數字,\"orders\":數字,\"days_with_data\":數字,\"best_day\":{\"date\":\"YYYY-MM-DD\",\"revenue\":數字},\"worst_day\":{\"date\":\"YYYY-MM-DD\",\"revenue\":數字},\"avg_daily_revenue\":數字,\"notes\":\"該月觀察30字內\"}],\"total_revenue\":數字,\"total_orders\":數字,\"total_days\":數字,\"summary\":\"整體月份結算的3-5句總結\",\"best_month\":{\"month\":\"YYYY-MM\",\"revenue\":數字},\"worst_month\":{\"month\":\"YYYY-MM\",\"revenue\":數字},\"key_findings\":[\"發現1\",\"發現2\"]}";
...
router.get('/branch-source-aggregate/:branch', async (req, res) => {
  try {
    ...
    for (const r of all) {
      let content = '';
      const attUrl = r.attachment_url || '';
      const fname = attUrl ? path.basename(attUrl) : null;
      ...
      if (fpath && fs.existsSync(fpath)) {
        ...
        try {
          if (ext === '.xlsx' || ext === '.xls') {
            ...
            content = fs.readFileSync(fpath, 'utf8').slice(0, 18000);
          } else {
            content = '(非文字檔: ' + ext + ' — 用記錄摘要)';
          }
        } catch (e) { content = '(讀檔錯: ' + e.message + ')'; }
      }
      blocks.push('=== 報告 ' + r.report_date + ' / 檔案 ' + (r.source_file || '手填') + ' ===\n報告摘要: ' + (r.summary || '') + '\n原始內容:\n' + (content || '(無原始檔)') + '\n總營收(報告填寫): NT$' + r.revenue + ' / 訂單 ' + (r.orders || 0));
    }
    ...
    const respText = (result.content || []).map(c => c.text || '').join('');
    let aggregate = {};
    try { const m = respText.match(/\{[\s\S]*\}/); if (m) aggregate = JSON.parse(m[0]); }
    catch (e) { aggregate = { _raw: respText.slice(0, 500), _err: e.message }; }
