...
router.post('/reanalyze-all', async (req, res) => { try { const all = loadAll(); const seenUploads = new Set(); const targets = []; for (const r of all) { const key = r.upload_id || (r.source_file || '').split(' [')[0]; if (seenUploads.has(key)) continue; if (r.expanded_from) continue; seenUploads.add(key); targets.push(r); } const results = { total: targets.length, expanded: 0, skipped: 0, errors: [] }; for (const orig of targets) { try { if (!orig.attachment_url) { results.skipped++; continue; } const fpath = path.join(UPLOADS_DIR, path.basename(orig.attachment_url)); if (!fs.existsSync(fpath)) { results.skipped++; continue; } const ext = (path.extname(orig.source_file || '') || '').toLowerCase(); let content = ''; if (ext === '.xlsx' || ext === '.xls') { const wb = new ExcelJS.Workbook(); await wb.xlsx.readFile(fpath); const rows = []; wb.eachSheet(sheet => { rows.push('=== ' + sheet.name + ' ==='); sheet.eachRow(row => { rows.push((row.values || []).slice(1).map(v => (v === null || v === undefined) ? '' : String(v)).join('	')); }); }); content = rows.join('
').slice(0, 30000); } else if (ext === '.csv' || ext === '.txt' || ext === '.md') { content = fs.readFileSync(fpath, 'utf8').slice(0, 30000); } else { results.skipped++; continue; } const result = await anthropic.messages.create({ model: 'claude-sonnet-4-5', max_tokens: 8192, system: REANALYZE_SYS, messages: [{ role: 'user', content: '門市: ' + (orig.branch || '') + '
原始檔: ' + (orig.source_file || '') + '

內容:
' + content }] }); const respText = (result.content || []).map(c => c.text || '').join(''); let parsed = {}; try { const m = respText.match(/\{[\s\S]*\}/); if (m) parsed = JSON.parse(m[0]); } catch (e) { results.errors.push({ branch: orig.branch, error: 'parse: ' + e.message }); continue; } const days = Array.isArray(parsed.daily) ? parsed.daily : []; if (days.length === 0) { results.errors.push({ branch: orig.branch, error: 'no days' }); continue; } const current = loadAll(); const cleanFile = (orig.source_file || '').split(' [')[0]; const origKey = orig.upload_id || cleanFile; const remaining = current.filter(r => (r.upload_id || (r.source_file || '').split(' [')[0]) !== origKey); const uploadId = orig.upload_id || genUploadId(); const newRecords = days.map((d, i) => ({ id: genId(), upload_id: uploadId, ts: new Date().toISOString(), report_date: (d.date && /^\d{4}-\d{2}-\d{2}$/.test(d.date)) ? d.date : orig.report_date, type: 'daily', branch: orig.branch, author: orig.author || '', revenue: Number(d.revenue) || 0, orders: Number(d.orders) || 0, problems: '', review: '', action_items: '', notes: d.notes || '', summary: '', source_file: cleanFile + ' [日 ' + (i + 1) + '/' + days.length + ']', source_size: orig.source_size, source_mime: orig.source_mime, attachment_url: orig.attachment_url, expanded_from: orig.id })); saveAll(remaining.concat(newRecords)); results.expanded++; } catch (e) { results.errors.push({ branch: orig.branch, error: e.message }); } } res.json({ ok: true, results }); } catch (e) { res.status(500).json({ ok: false, error: e.message }); } });
