// offline-reports.js — 線下報告中心 v3 (檔案上傳 + Claude 自動萃取 + 門市月明細 + AI 深度分析)
...
function buildSummaryForAI() {
  ...
  by_branch.forEach(br => {
    ...
    br.by_month = Object.values(mm).sort((a, b) => b.month.localeCompare(a.month));
  });
...
const BRANCH_ANALYSIS_SYS = "你是溫點 WarmPlace 烘焙坊的資深營運顧問。針對單一門市的線下報告做深度分析。只回傳純 JSON,絕對不要 markdown 標記。欄位:{\"executive_summary\":\"3-5句執行摘要\",\"monthly_trend_analysis\":\"逐月營收/訂單趨勢評析(150字內)\",\"top_issues\":[{\"issue\":\"問題描述\",\"severity\":\"high|medium|low\",\"root_cause\":\"根本原因推斷\"}],\"recommendations\":[{\"priority\":\"P1|P2|P3\",\"title\":\"建議標題\",\"detail\":\"具體做法(80字內)\",\"expected_impact\":\"預期效益\"}],\"quick_wins\":[\"立刻可做的小改善\"],\"watch_outs\":[\"需注意的風險\"]}";
...
router.get('/branch-analysis/:branch', async (req, res) => {
  try {
    ...
    console.error('[offline-reports] branch-analysis err', e);
    res.status(500).json({ ok: false, error: e.message });
  }
});
