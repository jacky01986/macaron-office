// offline-reports.js v4 — 線下報告中心 (月目標 + 達成率 + 客單價 + 月增減 + AI 深度分析)
...
router.post('/upload-file', upload.single('file'), async (req, res) => {
  try {
    ...
    if (ext === '.pdf' || mime === 'application/pdf') {
      ...
      return res.status(400).json({ ok: false, error: '不支援的檔案類型: ' + (ext || mime) });
    }
...
function buildSummaryForAI() {
  ...
  d30.forEach(r => {
    ...
    branchMap[b].reports.push({
      id: r.id, date: r.report_date, type: r.type, revenue: r.revenue, orders: r.orders,
      summary: r.summary, problems: r.problems, review: r.review, action_items: r.action_items,
      notes: r.notes, source_file: r.source_file, attachment_url: r.attachment_url, author: r.author
    });
  });
  ...
  by_branch.forEach(br => {
    ...
    (br.reports || []).forEach(r => {
      ...
      mm[m].revenue += Number(r.revenue) || 0;
      mm[m].orders += Number(r.orders) || 0;
      mm[m].count++;
      if (r.problems) r.problems.split('\n').filter(Boolean).forEach(() => mm[m].problems_count++);
    });
    // sort asc to compute MoM
  ...
  return {
    ...
    total_reports: all.length, recent_30: d30.length, recent_7: d7.length,
    revenue_30, revenue_7, problem_count: recent_problems.length,
    recent_problems: recent_problems.slice(0, 15),
    action_items, revenue_trend: trend, by_branch, recent_reports_brief
  };
}

router.get('/summary', (req, res) => {
  try { res.json(buildSummaryForAI()); }
  catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});
...
router.get('/branch-analysis/:branch', async (req, res) => {
  try {
    ...
    const targets = loadTargets();
    const branchTargets = targets[wantedBranch] || {};
    const byMonth = {};
    all.forEach(r => {
