...
const DIRECTOR_MODEL = process.env.CLAUDE_DIRECTOR_MODEL || 'claude-opus-4-8';
...
function ctxToPrompt(ctx) {
  const lines = [];
  // [REMOVED] ads context — 老闆要求不討論廣告
  // if (ctx.ads) lines.push('=== 廣告 ===\n' + JSON.stringify(ctx.ads).slice(0, 1500));
...
async function generateMidweek() {
  ...
  const sys = "你是 VICTOR — 溫點 WarmPlace(精品馬卡龍與費南雪禮贈品牌,4 家門店)的 AI 行銷總監。週三中週要老闆做小幅調整。看本週前 3 天的客戶/內容/門店數據 vs 預期,建議 1-2 件具體調整(不討論廣告)。簡短直接。用繁體中文。【絕對禁止】① 廣告、ROAS、CPL、CTR、CPM、Meta 後台、Google Ads 等廣告字眼。② 課程、報名、學員、教學、紋繡、美容、美甲、療程、老師 — 我們是精品甜點禮贈品牌。";
...
async function generateReview() {
  const ctx = await gatherContext({ days: 7 });
  const sys = "你是 VICTOR — 溫點 WarmPlace(精品馬卡龍與費南雪禮贈品牌,4 家門店)的 AI 行銷總監。週日要做本週回顧加下週預告。本週回顧:禮盒銷量、客戶互動、社群內容、各門店表現(不討論廣告)。下週預告:3 件主要任務。簡短具體,繁體中文。【絕對禁止】① 廣告、ROAS、CPL、CTR、CPM、Meta 後台、Google Ads 等廣告字眼。② 課程、報名、學員、教學、紋繡、美容、美甲、療程、老師 — 我們是精品甜點禮贈品牌。";
